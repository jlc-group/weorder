...
async def run():
    ...
    async with httpx.AsyncClient(timeout=30.0) as c:
        ...
        # Fetch first page of returns - 30 days
        body = json.dumps({"page_size": 50, "update_time_ge": tfr, "update_time_lt": tto}, separators=(',', ':'))
        p = common.copy()
        ...
        data = r.json()
        returns = data.get("data", {}).get("returns", []) if data.get("data") else []
        total_count = data.get("data", {}).get("total_count", len(returns)) if data.get("data") else 0

        print(f"=== TikTok Returns Summary (30 Days) ===")
        print(f"API Total Returns: {total_count}")
        print(f"First Page Sample: {len(returns)} items")
        
        # Check against DB
        in_db = not_in_db = already_term = 0
        for ret in returns:
            oid = ret['order_id']
        ...
        print(f"In DB (non-terminal): {in_db}")
        print(f"Already RETURNED/CANCELLED: {already_term}")
        ...
        db_returned = cur.fetchone()[0]
        cur.execute("SELECT count(*) FROM order_header WHERE channel_code = 'tiktok' AND status_normalized = 'CANCELLED'")
        db_cancelled = cur.fetchone()[0]
        
        print(f"\n=== Current DB Status ===")
        print(f"TikTok RETURNED: {db_returned}")
        print(f"TikTok CANCELLED: {db_cancelled}")

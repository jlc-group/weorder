...
async def run():
    ...
    print("=== Checking Physical Returns DB Status ===")
    ...
    async with httpx.AsyncClient(timeout=30.0) as c:
        ...
        for ret in returns:
            ...
            if return_type == "RETURN_AND_REFUND":
                cur.execute("SELECT status_normalized, status_raw FROM order_header WHERE external_order_id = %s", (oid,))
                row = cur.fetchone()
                if row:
                    status = row[0]
        ...
        print("\nPhysical Returns (RETURN_AND_REFUND) DB Status Distribution:")

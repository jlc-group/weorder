...
BATCH_SIZE = 5000  # Smaller batch size for responsiveness

def backfill_batched_iterative():
    db = SessionLocal()
    try:
        total_processed = 0
        
        while True:
            # 1. Fetch next batch of IDs
            logger.info(f"Fetching next {BATCH_SIZE} orders...")
            fetch_sql = text(f"""
                SELECT id 
                FROM order_header 
                ...
                AND (discount_amount = 0 AND platform_discount_amount = 0)
                ORDER BY id
                LIMIT {BATCH_SIZE}
            """)
            ...
            if not batch_ids:
                logger.info("No more orders to update. Finished.")
                break
                
            batch_ids_str = ",".join([f"'{uid}'" for uid in batch_ids])
            logger.info(f"Processing batch of {len(batch_ids)} orders...")

...
async def probe_endpoint(path, method="GET", params=None, body=None):
    ...
    if shop_cipher:
        combined_params["shop_cipher"] = shop_cipher
...
async def main():
    ...
    print(f"=== Checking Regular Order Status for {order_id} ===")
    data = await probe_endpoint("/order/202309/orders", method="GET", params={"ids": order_id})
    if data.get("code") == 0:
        orders = data.get("data", {}).get("orders", []) or data.get("data", {}).get("order_list", [])
        if orders:
            order = orders[0]
            print(f"Status: {order.get('status')}")
            print(f"Update Time: {datetime.fromtimestamp(order.get('update_time'))}")
        else:

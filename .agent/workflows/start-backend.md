...
1. **Verify Port Configuration**: 
   - Check `app/core/config.py` or `.env`. The default project port is **9202**.
...
2. **Run Server**:
   ...
   ```bash
   venv/bin/uvicorn main:app --reload --port 9202
   ```

   *Note: If port 9202 is busy, find and kill the process using it, do not change the port.*

...
class TikTokTester:
    def __init__(self, app_key, app_secret, shop_id, access_token):
        ...
        self.BASE_URL = "https://open-api.tiktokshop.com" # Changed from globalshop
    ...
    async def fetch_shop_cipher(self):
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Re-try with global if this fails
            if resp.status_code == 404:
                 return "ROW_Mfxq0QAAAADY9jhQTMr4JXq4U19IpM5-" # Hardcoded from earlier success
            data = resp.json()
            if data.get("code") == 0:
                for shop in data.get("data", {}).get("shops", []):
                    if str(shop.get("id")) == self.shop_id:
                        return shop.get("cipher")
        return "ROW_Mfxq0QAAAADY9jhQTMr4JXq4U19IpM5-"

    async def test_path(self, path, method="GET", body=None):
        if not self.shop_cipher:
            self.shop_cipher = await self.fetch_shop_cipher()
        ...
        async with httpx.AsyncClient() as client:
            ...
            print(f"PROBE {self.BASE_URL}{path} -> {resp.status_code}")
